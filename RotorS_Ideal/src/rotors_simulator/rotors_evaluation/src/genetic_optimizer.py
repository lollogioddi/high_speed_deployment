...
# Initialize ROS node
rospy.init_node('genetic_algorithm_optimizer', anonymous=True)
...
class MetricsListener:
    def __init__(self):
        ...
        self.overshoots = []
        self.settling_times = []
        rospy.Subscriber('/pelican_1/performance_metrics', Float64MultiArray, self.callback)
...
def evaluate(individual):
    for attempt in range(RETRY_LIMIT):
        try:
            ...
            yaml_path = '/your/directory/catkin_ws/src/rotors_simulator/rotors_gazebo/resource/lee_controller_pelican_for_tuning.yaml'
            ...
            # Stop the simulation
            if not call_service('/stop_simulation', Trigger):
                rospy.logerr("Failed to stop simulation: large cost returned")
                continue

            # Reset the simulation
            if not call_service('/reset_simulation', Trigger):
                rospy.logerr("Failed to reset simulation: large cost returned")
                continue

            # Signal data_saver to compute metrics
            stop_sim_pub = rospy.Publisher('/pelican_1/stop_simulation_signal', Bool, queue_size=10)
            ...
            shutdown_pub = rospy.Publisher('/pelican_1/shutdown_data_saver', Bool, queue_size=10)
            rospy.sleep(1)  # Give some time for the publisher to connect
...
def log_results(logbook, filename='genetic_algorithm_log.csv'):
...
def main():
    ...
    # Log the results
    log_results(logbook, filename='/your/directory/genetic_algorithm_log.csv')
