...
LeePositionController::LeePositionController()
  ...
  //std::cout << "R col2: " << odometry_.orientation.toRotationMatrix().col(2)<< std::endl; //debugging
LeePositionController::~LeePositionController() {}
...
void LeePositionController::ComputeDesiredAngularAcc(const Eigen::Vector3d& acceleration,
  ...
  // // Define roll, pitch, and yaw in radians
  // double roll = 0.0;  
  // double pitch = - M_PI/6;
  // double yaw = 1.0;
  // // Create rotation matrices from roll, pitch, and yaw
  // Eigen::Matrix3d rollMatrix = Eigen::AngleAxisd(roll, Eigen::Vector3d::UnitX()).toRotationMatrix();
  // Eigen::Matrix3d pitchMatrix = Eigen::AngleAxisd(pitch, Eigen::Vector3d::UnitY()).toRotationMatrix();
  // Eigen::Matrix3d yawMatrix = Eigen::AngleAxisd(yaw, Eigen::Vector3d::UnitZ()).toRotationMatrix();

  // // Combine the rotations
  // Eigen::Matrix3d R_des = yawMatrix * pitchMatrix * rollMatrix;
