...
class SMControl:
    ...
    def __init__(self):
        try:
            ...
            # parameters for checking attitude stabilization and switch control strategy
    ...
    def read_ros_params(self):
        try:
            ...
            k_pos_Lee = rospy.get_param('~position_gain')
            k_vel_Lee = rospy.get_param('~velocity_gain')
    ...
    # controller part that deals with the position in space. It activates only after the attitude stabilization
    # there is only one position controller even if there are two control strategies because it activates only after attitude stabilization and therefore controller switch
    ...
    # controller part that deals with the attitude (roll, pitch and yaw) of the drone. It uses Lee Geometric approach for control
    def attitude_controller_Lee(self, R, command_angular_velocity, angular_velocity, acceleration, b1_des):
        try:
            ...
            angular_acceleration = -np.multiply(angol_error_vector, self.normalized_k_att_Lee) -np.multiply(angular_rate_error,self.normalized_k_ang_rate_Lee) + gyroscopic_term
    ...
    # controller part that deals with the attitude (roll, pitch and yaw) of the drone. It uses SMC approach for control law
    ...
    def control_loop(self, allocation_matrix):
         while not rospy.is_shutdown():
            ...
            try:
                ...
                # 1. the system is not flagged as stabilized, let's check the angular stabilization
                # and if check_angular_stabilization is true then we use the Lee Geometric control
                if not self.is_stabilized and self.check_angular_stabilization(angular_velocity):
                    ...
                    torque, angol_error = self.attitude_controller_Lee(R, command_angular_velocity, angular_velocity, total_force, b1_des)
                    self.pub_angular_error.publish(Vector3(angol_error[0], angol_error[1], angol_error[2]))
                    self.start_time = rospy.Time.now().to_sec()
                # 2. the system is already flagged as stabilized so there is no need to check_angular_stabilization
                # we use Lee Geometric with position control
                elif self.is_stabilized:
                    # Use the stabilized controller
                    current_time = rospy.Time.now().to_sec()
                    # alpha is used as smoothing factor to blend together the control action of the two strategies and smooth the switch
                    alpha = self.compute_weighting_factor(current_time - self.start_time, 1.5)
                    total_force, position_error = self.position_controller(R, cos_theta_xy, position, command_position, linear_velocity, command_linear_velocity)
                    torque_aggressive, _ = self.attitude_controller(command_orientation, orientation, angular_velocity)
                    torque_stable, angol_error = self.attitude_controller_Lee(R, command_angular_velocity, angular_velocity, total_force, b1_des)

                    torque = alpha * torque_aggressive + (1 - alpha) * torque_stable
                    self.pub_angular_error.publish(Vector3(angol_error[0], angol_error[1], angol_error[2]))
                # 3. the system is neither flagged as stabilized nor check_angular_stabilization comes out true
                # we use aggressive flight SMC with fixed acceleration (position control not active)
                else:
                    # Use the Sliding Mode Control
...
def main():
    try:
        ...
        # allocation matrix is computed here since being constant and we don't need it to be computed more than once
