...
class AdaptiveControl:
    ...
    def __init__(self):
        try:
            ...
            # parameters for the switching mechanism
    ...
    def attitude_controller(self, R, command_orientation, command_angular_velocity, angular_velocity, acceleration, b1_des, k_att, k_ang_rate ):
        # controller part that deals with the attitude (roll, pitch and yaw) of the drone
        try:
            ...
            angular_acceleration = -np.multiply(angol_error_vector, k_att) -np.multiply(angular_rate_error,k_ang_rate) + gyroscopic_term
    ...
    def calculate_rotor_velocities(self, R, angular_acc_to_rotor_velocities, acceleration, angular_acceleration):
        # it uses the allocation matrix, the vertical acceleration and the 4 rotors thrusts to compute the rotor velocities
        try:
    ...
    def control_loop(self, allocation_matrix):
         while not rospy.is_shutdown():
            ...
            try:
                ...
                # and if check_angular_stabilization is true then we use the stable flight gains
                if not self.is_stabilized and self.check_angular_stabilization(angular_velocity):
                    # The drone is considered to be stabilizing; activates position control and use the computed acceleration
                ...
                # we use stable flight gains with position control
                ...
                # we use aggressive flight gains with fixed acceleration (position control not active)
                else:
